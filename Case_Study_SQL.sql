...
SELECT DATE_FORMAT(start_date, '%Y-%m-01') AS start_month, COUNT(*) AS trial_starts
FROM subscriptions
WHERE plan_id = 0
...
SELECT 
    COUNT(DISTINCT s1.customer_id) AS churned_after_trial,
...
SELECT 
    COUNT(*) AS plan_count,
    ROUND(COUNT(*) / (SELECT COUNT(*) FROM subscriptions WHERE plan_id = 0) * 100) AS plan_percentage
FROM subscriptions
WHERE plan_id != 0;
...
SELECT 
    AVG(DATEDIFF(s2.start_date, s1.start_date)) AS avg_days_to_annual_plan
FROM subscriptions s1
JOIN subscriptions s2 ON s1.customer_id = s2.customer_id
WHERE s1.plan_id = 0
AND s2.plan_id = 3
...
SELECT COUNT(DISTINCT s1.customer_id) AS downgrades_count
FROM subscriptions s1
JOIN subscriptions s2 ON s1.customer_id = s2.customer_id
WHERE s1.plan_id = 2
AND s2.plan_id = 1
